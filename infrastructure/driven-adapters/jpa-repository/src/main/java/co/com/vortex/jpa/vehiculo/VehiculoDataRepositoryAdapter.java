...
@Repository
public class VehiculoDataRepositoryAdapter extends AdapterOperations<Vehiculo, VehiculoData, Integer, VehiculoDataRepository>
        implements VehiculoRepository
{

    private final ConductorDataRepository conductorDataRepository;
    public VehiculoDataRepositoryAdapter(VehiculoDataRepository repository, ObjectMapper mapper, ConductorDataRepository conductorDataRepository) {
    ...
    public Vehiculo asociarConductorAVehiculo(int vehiculoId, int conductorId) {
       try {
           VehiculoData vehiculo = repository.findById(vehiculoId).orElseThrow(()->new IllegalQueryOperationException("No fue posible encontrar el vehiculo a asociar"));
           ConductorData conductor =conductorDataRepository.findById(conductorId).orElseThrow(()->new IllegalQueryOperationException("No fue posible encontrar el conductor a asociar"));
    ...
    public Vehiculo desasociarConductorDeVehiculo(int vehiculoId, int conductorId) {
       try {
           VehiculoData vehiculo = repository.findById(vehiculoId).orElseThrow(()->new IllegalQueryOperationException("No fue posible encontrar el vehiculo a asociar"));
           ConductorData conductor =conductorDataRepository.findById(conductorId).orElseThrow(()->new IllegalQueryOperationException("No fue posible encontrar el conductor a asociar"));
           if(vehiculo.getConductor().getId()==conductorId){
