...
private final DesasociarConductorDeVehiculoUseCase desasociarConductorDeVehiculoUseCase;
    ...
    public ResponseEntity<Vehiculo> registrarVehiculo(@RequestBody Vehiculo vehiculo) {
      try {
          return ResponseHandler.generateResponse(registrarVehiculoParaPedidosUseCase.registraVehiculo(vehiculo), HttpStatus.OK);
...
try {
    return ResponseHandler.generateResponse(verVehiculosAsociadosAConductorUseCase.listarVehiculosAsociados(idConductor),HttpStatus.OK);
...
try {
    return ResponseHandler.generateResponse(verVehiculosSinAsociarAConductorUseCase.listarVehiculosSinAsociar(idConductor),HttpStatus.OK);

}catch (Exception error){
    throw new RuntimeException("No fue posible listar los vehiculos disponibles para asociar al conductor",error);
}
    }

    @PostMapping(path = "/asociar/vehiculo/{idVehiculo}/conductor/{idConductor}")
    public ResponseEntity<Vehiculo> asociarConductorAVehiculo(@PathVariable int idVehiculo, @PathVariable int idConductor){
       try {
           return ResponseHandler.generateResponse(asociarConductorAVehiculoUseCase.asociarConductorAVehiculo(idVehiculo,idConductor),HttpStatus.OK);
       }catch (Exception error){
throw new RuntimeException("No fue posible asociar el conductor al vehiculo",error);
    ...
    public ResponseEntity<Vehiculo> desasociarConductorDeVehiculo(@PathVariable int idVehiculo, @PathVariable int idConductor){
try {
